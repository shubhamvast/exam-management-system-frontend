      ...
      <table className="table-auto border-collapse  w-full mt-2 ">
        <thead>
          <tr className="bg-[#172337] text-white h-10 flex justify-between items-center pr-24">
            {head.map((h,index) => {
              return (
                <th key={index} className=" text-left pl-6 font-thin text-gray-100">{h}</th>
              );
            })}
        ...
        <tbody>
          {items.map((item, index) => {
            return (
              <tr key={index} className=" h-16  flex justify-between items-center pr-20 border-b-2 border-slate-100">
